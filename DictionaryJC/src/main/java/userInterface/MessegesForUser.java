...
public enum MessegesForUser {
    MAINMENU("Просмотреть словарь - нажмите 1;" +
    ...
    REPORT_LANGUAGE_DICTIONARY_FILE_CREATION_FAILED("Не удалось создать файл LanguageDictionary.txt " +
            "(class CheckFunctions метод checkFileExistence)"),

    REPORT_NUMERIC_DICTIONARY_FILE_CREATION("Файл NumericDictionary.txt не был найден, создан новый файл."),

    REPORT_NUMERIC_DICTIONARY_FILE_CREATION_FAILED("Не удалось создать файл NumericDictionary.txt " +
            "(public class CheckFunctions метод checkFileExistence)"),
